...
def recommend(user, books, users, ratings):
    ...
    for other_user in users:
        if other_user != user:
            ...
            for i in range(len(books)):
                similarity += ratings[user][i] * ratings[other_user][i]
    ...
    for book in books:
        ...
        for similarity, other_user in top_similar_users:
            if ratings[other_user][books.index(book)] != 0:
                total_rating += ratings[other_user][books.index(book)]
                count += 1
        if count > 0:
            average_rating = total_rating / count
            recommended_books.append((average_rating, book))
